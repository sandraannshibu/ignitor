...
public class DeliveryViewpayment extends AppCompatActivity implements JsonResponse {
    ListView l1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_delivery_viewpayment);
        l1=(ListView) findViewById(R.id.list);


        sh= PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
        JsonReq JR = new JsonReq();
        JR.json_response = (JsonResponse) DeliveryViewpayment.this;
        String q = "/DeliveryViewpayment?log_id=" +sh.getString("log_id", "") + "&oid="+Deliveryviewrequest.oid;
        q = q.replace(" ", "%20");
        JR.execute(q);
    }
        ...
        try {
            ...
            if (status.equalsIgnoreCase("success")) {
                ...
                for (int i = 0; i < ja1.length(); i++) {
                    ...
                    request_id[i] = ja1.getJSONObject(i).getString("order_id");
